...
module.exports = {
	...
	parameters: {
		member: {
			requirement: true,
			description: "The member to kick out of the server given the member is in the guild. This argument must be the ID of the user or a user mention."
		},
		reason: {
			requirement: false,
			description: "The reason for the kick. The reason is not required to be put in double quotation marks and can have more than 1 word."
		}
	},
		...
		if(message.guild.me.hasPermission("KICK_MEMBERS")){ // Ensure the bot has permission to kick.
			switch(otherArguments.length){
				...
				case 1: // Only the user is supplied.
				{
					...
					if(!obtainedUserID) return message.reply("Sorry, invalid argument for member!");
				...
				default: // A reason is supplied as well as the user.
				{
					...
					// Obtain member to kick given valid argument.
					if(obtainedUserID.length === 0) return message.reply("Sorry, invalid argument for member!");
					const memberToKick = message.guild.members.get(obtainedUserID[0]);
...
function attemptKickMember(message, targetMember, reason){
	...
	if(targetMember.kickable){ // Ensure that the bot can kick the member.
		if(isAdministrator(targetMember)){ // Bot will never kick administrators.
			return message.reply("Sorry, I do not kick administrators of this server.");
		}
	...
	*/
	...
	if(botPermissions.has(["SEND_MESSAGES", "VIEW_CHANNEL"])){
		if(botPermissions.has("EMBED_LINKS")){ // Ensure can post embeds.
			...
			const roleUserString = isGuildOwner(message.guild, authorID) ? "Guild Owner" : isAdministrator(message.member) ? "Administrator" : "Moderator";
		...
		const messageString = "I require permission `EMBED_LINKS` to post `user_kicked` logs. I have disabled logging this for now, you will have to re-enable this setting.";
		return channelFound.send(messageString).then(() => targetMember.kick(reason));
	}
	// Unable to send message in logging channel.
	updateData([[guildIDToCheck, "UK", null]]);
	const errorMessage = `Cannot log \`user_kicked\` logs to <#${channelFound.id}> as I do not have \`SEND_MESSAGES\` or \`READ_MESSAGES\` permissions.\nPlease ask an administrator to re-setup for logging \`user_kicked\` if needed.`;
